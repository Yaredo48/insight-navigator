-- Create troubleshooting_sessions table to track active sessions (if not exists)
...
-- Create user_progress table for gamification (badges, tips, stats) (if not exists)
...
-- Create troubleshooting_templates table for predefined flows (if not exists)
CREATE TABLE IF NOT EXISTS public.troubleshooting_templates (
    id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
    title TEXT NOT NULL,
...
-- Drop existing policies first (PostgreSQL doesn't support IF NOT EXISTS for CREATE POLICY)
DROP POLICY IF EXISTS "Anyone can view troubleshooting sessions" ON public.troubleshooting_sessions;
DROP POLICY IF EXISTS "Anyone can create troubleshooting sessions" ON public.troubleshooting_sessions;
DROP POLICY IF EXISTS "Anyone can update troubleshooting sessions" ON public.troubleshooting_sessions;
DROP POLICY IF EXISTS "Anyone can delete troubleshooting sessions" ON public.troubleshooting_sessions;
...
DROP POLICY IF EXISTS "Anyone can view troubleshooting templates" ON public.troubleshooting_templates;
DROP POLICY IF EXISTS "Anyone can create troubleshooting templates" ON public.troubleshooting_templates;
...
DROP TRIGGER IF EXISTS update_troubleshooting_templates_updated_at ON public.troubleshooting_templates;
CREATE TRIGGER update_troubleshooting_templates_updated_at
    BEFORE UPDATE ON public.troubleshooting_templates
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

-- Seed with initial troubleshooting templates (idempotent - skip if exists)
INSERT INTO public.troubleshooting_templates (title, description, category, steps)
SELECT * FROM (VALUES
    (
...
) AS t(title, description, category, steps)
WHERE NOT EXISTS (SELECT 1 FROM public.troubleshooting_templates WHERE title = t.title);